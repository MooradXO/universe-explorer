...
export interface FlightSave { version: 1; address: StellarAddress; spectrum: string | null; rotation: number[]; visited: string[]; }
...
export function readFlightSave(raw: string | null): FlightSave | null {
  ...
  try {
    ...
    if (value.version !== 1 || !address || (value.spectrum !== null && (typeof value.spectrum !== 'string' || value.spectrum.length > 128)) ||
    ...
    return { version: 1, address, spectrum: value.spectrum, rotation: value.rotation, visited: value.visited };
  } catch { return null; }
}
