...
export class SystemScene {
  ...
  update(dt: number, elapsed: number, origin: FloatingOrigin, focus: THREE.Vector3) {
    ...
    if (this.selectionTime >= 0.2) {
      ...
      const wanted = this.descriptor.bodies.filter(body => Math.hypot(...body.position.map((v, i) => v - absoluteShip[i])) < SYSTEM_CONFIG.renderDistance)
        .sort((a, b) => this.distance(a, absoluteShip) - this.distance(b, absoluteShip)).slice(0, Settings.graphicsMode === 'LOW' ? 2 : 4);
      ...
      if(nearest)this.backdrop.setEnvironment(nearest.environment);
      ...
      for(const {body,zone} of nearby)if(!this.debris.has(zone.id)){const visual=new OrbitalDebris(body,Settings.graphicsMode==='LOW',this.targets,zone);this.group.add(visual.group);this.debris.set(zone.id,visual);}
    ...
    for (const body of this.descriptor.bodies) {
      ...
      if (planet) { planet.group.position.set(...origin.toLocal(worldPosition(undefined, body.position))); planet.update(elapsed); }
    }
  ...
  snapshot() { return { id: this.descriptor.anchor.catalogId, title: this.descriptor.anchor.title, spectrum: this.descriptor.spectrum,
    ...
    visualStyle:'cinematic',decorativeBackground:true,backdrop:this.backdrop.snapshot(),debris:[...this.debris.values()][0]?.snapshot()??null,
    zones:[...this.debris.values()].map(zone=>zone.snapshot()),
    openSpace:this.openSpace.snapshot(),
    bodies: this.descriptor.bodies.map(body => ({ id: body.id, name: body.name, origin: body.origin, position: [...body.position], radius: body.radius, semiMajorAu: body.orbit.semiMajorAu,
      environment:body.environment,satellites:this.planets.get(body.id)?.snapshot().satellites??[] })) }; }
  dispose() {
    this.removeTarget(this.star); this.starGeometry.dispose(); this.starMaterial.dispose();
    this.glowTexture.dispose();this.glowMaterial.dispose();this.backdrop.dispose();this.openSpace.dispose();for(const zone of this.debris.values())zone.dispose();this.debris.clear();
