...
export function createSpacePlanetTexture(descriptor: PlanetDescriptor, baseColorHex: number): THREE.Texture {
  ...
  const rng = createSeededRandom(hashString(`${descriptor.planetId}:${descriptor.seed}:space-surface`));
  ctx.fillStyle = colorToCss(baseColorHex);
  ctx.fillRect(0, 0, canvas.width, canvas.height);

  if (descriptor.biome === 'gas') {
    ...
    const oceanLike = descriptor.biome === 'forest' || descriptor.biome === 'oceanic' || descriptor.biome === 'ice';
    if (oceanLike) {
      ctx.fillStyle = colorToCss(descriptor.visual.waterColor);
      ctx.fillRect(0, 0, canvas.width, canvas.height);
    }
    ...
    for (let land = 0; land < landCount; land++) {
      ...
      const rx = 28 + rng() * 68;
      const ry = 16 + rng() * 42;
      ctx.fillStyle = colorToCss(descriptor.visual.landColor);
      ctx.beginPath();
      ctx.ellipse(cx, cy, rx, ry, rng() * Math.PI, 0, Math.PI * 2);
      ctx.fill();

      ctx.fillStyle = jitterColor(descriptor.visual.surfaceTint, (rng() - 0.5) * 0.12);
      ctx.beginPath();
      ctx.ellipse(cx + (rng() - 0.5) * rx * 0.45, cy + (rng() - 0.5) * ry * 0.45, rx * 0.36, ry * 0.38, rng() * Math.PI, 0, Math.PI * 2);
      ctx.fill();
    }

    if (descriptor.biome === 'volcanic') {
...
export function createSpaceCloudTexture(descriptor: PlanetDescriptor): THREE.Texture {
  const canvas = document.createElement('canvas');
  canvas.width = 256;
  canvas.height = 128;
  const ctx = canvas.getContext('2d');
  ...
  const rng = createSeededRandom(hashString(`${descriptor.planetId}:${descriptor.seed}:space-clouds`));
  ctx.clearRect(0, 0, canvas.width, canvas.height);
  const cloud = new THREE.Color(descriptor.visual.cloudColor);
  const alpha = descriptor.visual.cloudCoverage * (descriptor.biome === 'gas' ? 0.55 : 0.72);

  ctx.fillStyle = `rgba(${Math.floor(cloud.r * 255)},${Math.floor(cloud.g * 255)},${Math.floor(cloud.b * 255)},${alpha})`;
  const count = 8 + Math.floor(descriptor.visual.cloudCoverage * 16);
