...
export const PlanetShaders = {
  vertex: `
    ...
    void main() {
      ...
      // Determine terrain height - smoother multi-octave
      float n = snoise(position * 0.1) * 0.5 + 0.5; // base continents
      n += snoise(position * 0.3) * 0.25; // details
      vNoise = n;
      
      vec3 pos = position;
      // Soft gentle displacement for mountains (max 0.4 units)
      if(n > 0.55) {
          float bump = smoothstep(0.55, 1.0, n) * 0.5;
          pos += normal * bump;
      }
  ...
  fragment: `
    ...
    void main() {
      // Basic lighting (directional from star)
      vec3 lightDir = normalize(lightPosition - vPosition);
      float lambert = max(dot(normalize(vNormal), lightDir), 0.05); 
      
      // Biome blending
      vec3 finalColor = oceanColor;
      if(vNoise > 0.55) {
         float mnt = smoothstep(0.55, 0.8, vNoise);
         finalColor = mix(landColor, vec3(0.9, 0.9, 0.9), mnt); // Blend to snow/grey tips
      } else {
         // Coastlines and deep water
         float depth = smoothstep(0.0, 0.55, vNoise);
         finalColor = mix(oceanColor * 0.5, oceanColor, depth);
      }
      
      gl_FragColor = vec4(finalColor * lambert, 1.0);
    }
