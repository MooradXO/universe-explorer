...
import { isGameMode, roomForMode, type GameMode } from '../src/network/shared/GameMode';
...
export class UniverseRoom extends Room {
  ...
  static readonly instances = new Map<GameMode, UniverseRoom>();
  static get instance() { return this.instances.get('pvp'); }
  mode!: GameMode;
  ...
  onCreate(options: { canonical?: boolean; mode?: GameMode }) {
    const mode = options.mode ?? 'pvp';
    if (!options.canonical || !isGameMode(mode) || UniverseRoom.instances.has(mode)) throw new Error('Only canonical game modes are available');
    this.mode = mode; UniverseRoom.instances.set(mode, this);
    this.roomId = roomForMode(mode); this.maxClients = UniverseRoom.services.maxPlayers ?? NET.maxPlayers; this.autoDispose = false; this.maxMessagesPerSecond = 150;
  ...
  onAuth(_client: Client, options: Record<string, unknown>) {
    ...
    if ((options.mode ?? 'pvp') !== this.mode) throw new Error('Game mode mismatch');
    const identity = UniverseRoom.services.guests.identity(options.token, this.mode);
    this.checkAdmission(identity.id);
  ...
  onJoin(client: Client) {
    ...
    // Synchronous admission after authentication also covers concurrent joins into different rooms.
    this.checkAdmission(auth.id);
    ...
    client.send('welcome', { id: auth.id, token: auth.token, version: NET.version, generator: GENERATOR, mode: this.mode }); this.game.announce(pilot);
  }
  private checkAdmission(id: string) {
    const rooms = [...UniverseRoom.instances.values()];
    if (rooms.some(room => room.game.pilots.has(id))) throw new Error('This guest is already connected. Close the other tab or reconnect.');
    if (rooms.reduce((total, room) => total + room.game.pilots.size, 0) >= this.maxClients) throw new Error('Universe full');
  }
  ...
  onReconnect(client: Client) {
    ...
    client.send('welcome', { id: auth.id, token: auth.token, version: NET.version, generator: GENERATOR, mode: this.mode }); this.game.reconnect(auth.id);
  }
  onLeave(client: Client) {
    const auth = client.auth as GuestAuth; if (!auth || !this.joined.delete(client.sessionId)) return;
    const pilot = this.game.pilots.get(auth.id);
    if (pilot) UniverseRoom.services.guests.save(auth.token, this.game.self(pilot), this.mode);
  ...
  private save() {
    for (const [id, client] of this.clientsById) {
      const p = this.game.pilots.get(id); if (p) UniverseRoom.services.guests.save((client.auth as GuestAuth).token, this.game.self(p), this.mode);
    }
    UniverseRoom.services.guests.flush();
  }
  onDispose() { this.save(); UniverseRoom.instances.delete(this.mode); }
  ...
  metrics() {
    ...
    return { mode: this.mode, players: this.game.pilots.size, maxPlayers: this.maxClients, systems: this.game.systems.size, bots: [...this.game.systems.values()].reduce((n, s) => n + s.bots.entities.size, 0),
